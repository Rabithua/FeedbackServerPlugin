...
4. The Agent keeps the returned ten-minute connection code available and immediately retries
   `connection_status`, causing the host to open FeedbackKit OAuth once.
5. If the page already shows the invited email, entitlement, client, and requested scopes, continue
   directly. If it asks for a connection code, paste the ten characters; this fallback binds an
   isolated browser and is not another identity check.
...
approval. Same-browser handoffs bind automatically; isolated WebViews and different browsers use
the short-code fallback.
