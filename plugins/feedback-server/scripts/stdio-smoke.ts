...
const client = new Client({ name: 'feedback-server-stdio-smoke', version: '0.10.1' });

try {
  await client.connect(transport);
  const tools = (await client.listTools()).tools;
  if (tools.length !== 72) {
    throw new Error(`Expected 72 FeedbackServer tools, received ${tools.length}`);
  }
  ...
  console.error('FeedbackServer stdio bundle initialized with 72 tools.');
} finally {
  await client.close();
}
