...
const client = new Client({ name: 'feedback-server-stdio-smoke', version: '0.9.2' });

try {
  await client.connect(transport);
  const tools = (await client.listTools()).tools;
  if (tools.length !== 69) {
    throw new Error(`Expected 69 FeedbackServer tools, received ${tools.length}`);
  }
  ...
  console.error('FeedbackServer stdio bundle initialized with 69 tools.');
} finally {
  await client.close();
}
